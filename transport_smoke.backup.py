...
def run_optional_order_test(transport: WebullBrokerTransport) -> None:
    ...
    if "sandbox" not in SDK_ENDPOINT.lower():
        raise RuntimeError(
            "Order smoke testing is restricted to the sandbox endpoint."
        )

    place_order = getattr(transport, "place_order", None)
    if not callable(place_order):
        print(
            "\nOrder test requested, but WebullBrokerTransport does not "
            "currently implement place_order()."
        )
        return

    print("\nSubmitting sandbox test order...")
    order = place_order(
        symbol="AAPL",
        side="BUY",
        quantity=1,
...
def main() -> None:
    ...
    if not access_token:
        raise RuntimeError("Webull SDK did not return an access token.")

    print("Access token initialized.")

    try:
        run_sdk_checks(sdk_client, ACCOUNT_ID)
    except Exception as exc:
        print("\nDirect SDK check failed:")
        print(type(exc).__name__, str(exc))

    credentials = create_credential_provider()
    ...
    try:
        print("\nAccount")
        print(transport.get_account())

        print("\nCash")
        print(transport.get_cash())

        print("\nPositions")
        print(transport.get_positions())

        run_optional_order_test(transport)
