...
def test_safe_default_frozen_roundtrip():
 p=OrderStatusPolicy();assert not p.enabled and not hasattr(p,"__dict__") and OrderStatusPolicy.from_dict(p.to_dict())==p
...
def test_policy_validation(kwargs):
 with pytest.raises(OrderStatusValidationError):OrderStatusPolicy(**kwargs)
