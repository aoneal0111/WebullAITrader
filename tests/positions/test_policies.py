...
def test_policy_default_frozen_roundtrip():
 p=PositionsPolicy();assert not p.enabled and not hasattr(p,"__dict__") and PositionsPolicy.from_dict(p.to_dict())==p
...
def test_policy_validation(kwargs):
 with pytest.raises(PositionsValidationError):PositionsPolicy(**kwargs)
