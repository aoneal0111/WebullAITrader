...
def test_public_factory_wraps_existing_order_without_copying() -> None:
    ...
    command = api.create_expire_command(
    ...
    assert command.command_type == "expire"
    assert command.command_id is command_id
    assert command.payload is order
    assert command.occurred_at is occurred_at


def test_expire_command_serialization_is_deterministic() -> None:
    first_order = make_order()
    second_order = make_order()

    first = api.create_expire_command(
        command_id="EXPIRE-1",
        order=first_order,
        occurred_at=NOW,
    )
    second = api.create_expire_command(
        command_id="EXPIRE-1",
        order=second_order,
        occurred_at=NOW,
    )
    ...
    assert first_serialized["command_type"] == "expire"
...
def test_factory_does_not_mutate_caller_owned_values() -> None:
    ...
    assert command_id == "EXPIRE-1"
    assert occurred_at is NOW
    assert command.payload is order


def test_expire_factory_test_imports_only_public_package_and_stdlib() -> None:
