...
def test_public_factory_wraps_existing_order_without_copying() -> None:
    order = make_order()
    command_id = "ACCEPT-1"
    occurred_at = NOW

    command = api.create_accept_command(
    ...
    assert command.command_type == "accept"
    assert command.command_id is command_id
    assert command.payload is order
    assert command.occurred_at is occurred_at


def test_accept_command_serialization_is_deterministic() -> None:
    first_order = make_order()
    second_order = make_order()

    first = api.create_accept_command(
        command_id="ACCEPT-1",
        order=first_order,
        occurred_at=NOW,
    )
    second = api.create_accept_command(
        command_id="ACCEPT-1",
        order=second_order,
        occurred_at=NOW,
    )
    ...
    assert first_serialized["command_type"] == "accept"
...
def test_factory_does_not_mutate_caller_owned_values() -> None:
    ...
    assert command_id == "ACCEPT-1"
    assert occurred_at is NOW
    assert command.payload is order


def test_accept_factory_test_imports_only_public_package_and_stdlib() -> None:
