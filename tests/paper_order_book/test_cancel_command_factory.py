...
def test_public_factory_wraps_existing_order_without_copying() -> None:
    ...
    command = api.create_cancel_command(
...
def test_cancel_command_serialization_is_deterministic() -> None:
    order = make_order()

    first = api.create_cancel_command(
        command_id="CANCEL-1",
        order=order,
        occurred_at=NOW,
    )
    second = api.create_cancel_command(
        command_id="CANCEL-1",
        order=order,
        occurred_at=NOW,
    )
    ...
    assert first_serialized["command_type"] == "cancel"
...
def test_factory_does_not_mutate_caller_owned_values() -> None:
    ...
    assert command_id == "CANCEL-1"
    assert occurred_at is NOW
    assert command.payload is order


def test_cancel_factory_test_imports_only_public_package_and_stdlib() -> None:
