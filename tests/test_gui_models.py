...
def test_initial_dashboard_snapshot_is_safe_and_stopped() -> None:
    ...
    assert snapshot.runtime.environment == "PAPER"
    assert snapshot.runtime.state is RuntimeState.STOPPED
    assert snapshot.runtime.broker_status == "Disconnected"
    assert snapshot.runtime.emergency_stop_enabled is True
    assert snapshot.runtime.cycle_count == 0
